function [Ynew,points,i,neb_flag,max_dist] =  GPR_ci_neb_opt_pinv(X,values,points,alpha,optimizedParams,K,max_iterations,max_dist_limit,ci_image)
tic
disp('Starting CI-NEB optimisation')
initial_point = points(ci_image,:);
...
sp_const = 1 * 0.009648; % from eV to gA/ft^2 mol
spring = spring_parallel(points,tan,sp_const);

%% Calculating NEB forces at points
[neb_f,f] = cineb_force(points,tan,spring,force,ci_image);
...
while  ((normforce(neb_it)> 0.005) && (i < max_iterations)) && (max_dist < max_dist_limit)
iter =i;
    if i>1
        % Fletcher Reeves
        gg=grad1;
    ...
    if an<90
        if i>1 %%% This is done to prevent accelerating in the first step...
            ...
            %FIRE_dt = min( FIRE_dt * 1.2, FIRE_dtmax )
        end
    ...
    [g_over,f_over] = cineb_force(p_new,tan,spring,force,ci_image);
    ...
    if ((ant>120))
        ...
        tan=improved_tan(p_new,energy);
        spring = spring_parallel(p_new,tan,sp_const);
        [g_new,f_new] = cineb_force(p_new,tan,spring,force,ci_image);
    ...
    v=v_new;
    final_point = points(ci_image,:);
    max_dist = norm(final_point-initial_point);
...
disp(['Number of CI-NEB evaluations = ', num2str(neb_it)])
disp('CI-NEB optimisation completed')
